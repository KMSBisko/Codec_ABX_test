"""Data models for the web ABX tester.

Ported from app/models.py with web-specific adaptations.
"""
...
class LabelMappingMode(str, Enum):
    FIXED = "fixed"
...
# ── Preprocess Status (for streaming progress) ───────────────────────────────
...
def codec_catalog() -> Dict[str, CodecProfile]:
    """Return the full catalog of available codec profiles."""
    return {
...
def trial_result_to_dict(trial: TrialResult) -> Dict[str, Any]:
    return asdict(trial)


def session_to_dict(session: PreparedSession) -> Dict[str, Any]:
    d = asdict(session)
    # Enum serialization
    if isinstance(d.get("mode"), SampleRateMode):
        d["mode"] = d["mode"].value
    if isinstance(d.get("processing_mode"), ProcessingMode):
        d["processing_mode"] = d["processing_mode"].value
    return d


def codec_catalog_to_json() -> str:
